...
export const getRandomMoviesList = async (yearRange, voteRange, genres = '') => {
  try {
    const { status: firstStatus, data: firstData } = await axios.get(
      `${themoviedbUri}/discover/movie?api_key=440220d48afb156283f60fe68cbb2920&language=ru-RU&include_adult=false&page=${random(1, 500)}&release_date.gte=${yearRange[0]}-01-01&release_date.lte=${yearRange[1]}-12-31&vote_average.gte=${voteRange[0]}&vote_average.lte=${voteRange[1]}&with_genres=${genres}`,
    );
    ...
    if (firstData.page > firstData.total_pages) {
      const { status, data } = await axios.get(
        ...
        &include_adult=false&page=${random(1, firstData.total_pages)}&release_date.gte=${yearRange[0]}&release_date.lte=${yearRange[1]}
